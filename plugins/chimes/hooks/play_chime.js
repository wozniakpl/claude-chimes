 ...
 * Pure Node.js stdlib only — no Python required.
 */
...
function renderWav(notes) {
  ...
  const release = Math.max(1, Math.floor(noteSamples * 0.30));
  ...
  for (const index of notes) {
    ...
    for (let i = 0; i < noteSamples; i++) {
      ...
      else if (i > noteSamples - release) env = Math.max(0, (noteSamples - i) / release);
      else env = 1.0;
      const s = Math.round(AMPLITUDE * env * Math.sin(2 * Math.PI * f * i / SAMPLE_RATE) * 32767);
...
function play(notes, wavPath) {
  if (process.platform === "win32") {
    ...
    const beeps = notes
      .map((i) => `[Console]::Beep(${Math.round(freqFor(i))}, ${durationMs}); Start-Sleep -Milliseconds ${gapMs}`)
      .join("; ");
    spawnSync("powershell", ["-NoProfile", "-NonInteractive", "-Command", beeps], { stdio: "ignore" });
  } else if (process.platform === "darwin") {
    const child = spawn("afplay", [wavPath], { stdio: "ignore", detached: true });
    child.unref();
  } else {
    const avail = (cmd) => spawnSync("which", [cmd], { stdio: "pipe" }).status === 0;
...
function run(stdinData) {
  let payload = {};
  try { if (stdinData.trim()) payload = JSON.parse(stdinData); } catch (_) {}
